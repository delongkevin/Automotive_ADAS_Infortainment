"""
Automation Framework – Live Test-Case Generator
=======================================================
...
def generate_session_script(
    ...
    lines = [
        "#!/usr/bin/env python3",
        '"""',
        f"Auto-generated Automation Framework session script",
        f"Suite  : {suite_name}",
        f"Created: {generated}",
        ...
        "# Walk upward from this script until we find a directory that contains",
        "# a 'Automation_Framework/__init__.py' package, then add it to",
        "# sys.path so that 'import Automation_Framework' works regardless",
        "# of where this script was placed.",
        "_here = Path(__file__).resolve().parent",
        "_fw_parent = _here",
        "while _fw_parent != _fw_parent.parent:",
        "    if (_fw_parent / 'Automation_Framework' / '__init__.py').is_file():",
        "        break",
        "    _fw_parent = _fw_parent.parent",
        "else:",
        "    raise RuntimeError(",
        "        f'Automation_Framework package not found searching upward from {_here}.'",
        "        ' Move this script to a subdirectory of the repository root.'",
        "    )",
        "if str(_fw_parent) not in sys.path:",
        "    sys.path.insert(0, str(_fw_parent))",
        "",
