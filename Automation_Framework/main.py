"""
Automation Framework – Main Execution Entry Point
=========================================================
...
# Layout: <repo_root>/Automation_Framework/main.py
# ---------------------------------------------------------------------------
_FRAMEWORK_DIR = Path(__file__).resolve().parent          # Automation_Framework/
...
def _discover_python_suites(directory: Optional[Path] = None) -> List[Path]:
    ...
    ----------
    directory:
        Directory to scan.  Defaults to ``Automation_Framework/tests/``
...
def _discover_json_files(directory: Optional[Path] = None) -> List[Path]:
    ...
    ----------
    directory:
        Directory to scan.  Defaults to ``Automation_Framework/``
        when *None*.  Pass any :class:`~pathlib.Path` to scan a different
        location.
    ...
    """
    from Automation_Framework.runner import discover_test_case_files
...
def _print_list(directory: Optional[Path] = None) -> None:
    ...
    print("\n╔══════════════════════════════════════════════════════════════╗")
    print(  "║          Automation Framework – Available Suites      ║")
    print(  "╚══════════════════════════════════════════════════════════════╝\n")
...
def _ensure_t32_running(
    ...
    *import time*, so Trace32 must already be listening on its RCL port
    ...
    """
    from Automation_Framework.core.connection import T32Connection
    from Automation_Framework.config import settings
    from Automation_Framework.utils.exceptions import T32ConnectionError
    ...
    if conn.try_connect():
        print(
            f"[Framework] Trace32 detected on port {port} "
    ...
    print(
        f"[Framework] Trace32 not found on port {port} – launching automatically …"
    )
    conn.launch()
    ...
    while _time.monotonic() < deadline:
        if conn.try_connect():
            conn.disconnect()
            print(f"[Framework] Trace32 ready on port {port}.")
            return
        _time.sleep(1.0)
...
def _run_python_suite(
    ...
    verbosity:
        Passed to :class:`~Automation_Framework.html_report.SanityHtmlRunner`.
    """
    from Automation_Framework.html_report import SanityHtmlRunner

    label      = _suite_label(suite_path)
    mode_upper = mode.upper()
    print(f"\n[Framework] Suite: {suite_path.name}  [mode={mode_upper}]")
...
def _run_all_python_suites(
    ...
    if not suites:
        print("[Framework] No Python test suites found in tests/.")
        return results
    for p in suites:
...
def _run_json_suite(
    ...
    """
    from Automation_Framework import runner as _runner
    ...
    if not matches:
        ...
        sys.exit(
            f"[Framework] ERROR: No JSON suite named '{label}' found.\n"
            f"         Available labels: {available or ['(none)']}"
        )

    json_path = matches[0]
    print(
        f"\n[Framework] Running JSON suite '{label}' from {json_path.name}  "
    ...
    print(f"[Framework] Suite '{label}' done: {report.summary()}")
...
def _run_all_json_suites(
    ...
    if not json_files:
        print(
            "[Framework] No '*_test_cases.json' files found. "
            "Add a JSON file (e.g. 'stress_test_cases.json') to run more suites."
        )
        return

    print(
        f"\n[Framework] Running all JSON suites  "
    ...
    print(f"\n[Framework] All JSON suites complete. {len(results)} suite(s) ran.")
...
def _run_discover(
    ...
    """
    from Automation_Framework.core.connection import T32Connection
    from Automation_Framework.core.symbol_discovery import (
        discover_symbols, DiscoveredSymbol, SymbolInventory,
    )
    from Automation_Framework.generator import generate_from_inventory
    from Automation_Framework.config import settings
    ...
    if verbose:
        print(f"[Framework][VERBOSE] Connected to Trace32 on port {port or settings.rcl_port}")
        try:
            t32_ver = conn.fnc("VERSION.BUILD()")
            print(f"[Framework][VERBOSE] Trace32 version: {t32_ver}")
        except Exception as exc:
            print(f"[Framework][VERBOSE] Could not retrieve T32 version: {exc}")

    try:
        print(f"\n[Framework] Discovering symbols  pattern={pattern!r} …")
        if verbose:
            print(f"[Framework][VERBOSE] Sending: SYMBOL.LIST {pattern}")
        ...
        print(f"[Framework] {inventory.summary()}")

        if verbose:
            for sym in inventory:
                print(
                    f"[Framework][VERBOSE]   {sym.kind.value:<10} {sym.name:<50} "
                    f"addr={sym.address or '?':>12}  size={sym.size}"
                )

        # Optional module filter
        if module_filter:
            print(f"[Framework] Applying module filter: {module_filter!r}")
            filtered = [
                s for s in inventory
                if module_filter.lower() in s.module.lower()
            ]
            inventory = SymbolInventory(filtered)
            print(f"[Framework] Filtered: {inventory.summary()}")

        # Optional one-shot breakpoint verification
        if breakpoint_symbol:
            from Automation_Framework.core import breakpoints as _bpm
            print(f"[Framework] Verifying breakpoint on '{breakpoint_symbol}' …")
            if verbose:
                print(f"[Framework][VERBOSE] Sending: BREAK.SET {breakpoint_symbol} /Program")
            try:
                _bpm.set_breakpoint(breakpoint_symbol, conn)
                print(f"[Framework] Breakpoint set on '{breakpoint_symbol}' ✔")
            except Exception as exc:
                print(f"[Framework] WARNING: breakpoint on '{breakpoint_symbol}' failed: {exc}")
    ...
    print(f"\n[Framework] ── Artefacts written ──────────────────────────────────")
    print(f"[Framework]   JSON test suite   → {result['json_path']}")
    print(f"[Framework]   Session script    → {result['script_path']}")
    print(f"[Framework] ───────────────────────────────────────────────────────")
    print(f"[Framework] Next steps:")
    print(f"[Framework]   1. Edit {Path(result['json_path']).name} to match your symbols")
    print(f"[Framework]   2a. Run via main.py: python main.py --json {result['json_path']}")
    print(f"[Framework]   2b. Run standalone: python {result['script_path']}")
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="python main.py",
        description=(
            "Automation Framework – test execution entry point.\n\n"
            "Examples:\n"
...
def main(argv: Optional[List[str]] = None) -> None:
    ...
    # ------------------------------------------------------------------
    if args.gui:
        from Automation_Framework.gui import main as _gui_main
        _gui_main()
        return
    ...
    if args.suite:
        if args.auto_launch and args.mode == "mock":
            print(
                "[Framework] Note: --auto-launch has no effect in mock mode "
        ...
        if args.suite.lower() == "all":
            ...
            if not matches:
                ...
                sys.exit(
                    f"[Framework] ERROR: No Python suite named '{args.suite}'.\n"
    ...
    if args.json:
        if args.mode == "mock":
            print(
                "[Framework] Note: JSON suites always connect to a real Trace32 "
