...
class TestTestCaseResult(unittest.TestCase):
    def test_to_dict_keys(self):
        from Automation_Framework.report import TestCaseResult
...
class TestLoadTestCases(unittest.TestCase):
    ...
    def test_load_basic(self):
        from Automation_Framework.runner import load_test_cases
...
class TestRunFromJson(unittest.TestCase):
    ...
    def test_run_skips_disabled(self):
        from Automation_Framework import runner as r
        tcs = [
        ...
        try:
            with (
                patch("Automation_Framework.runner.t32.T32Connection",
                      return_value=mock_conn),
                patch("Automation_Framework.runner.settings.load_from_json"),
            ):
...
class TestRunAllDiscovered(unittest.TestCase):
    ...
    def test_no_json_files_raises(self):
        from Automation_Framework.runner import run_all_discovered
        with tempfile.TemporaryDirectory() as tmp:
            with self.assertRaises(FileNotFoundError):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                patch("Automation_Framework.runner.t32.T32Connection",
                      return_value=mock_conn),
                patch("Automation_Framework.runner.settings.load_from_json"),
                patch("Automation_Framework.report.TestCaseReport.save_json"),
                patch("Automation_Framework.report.TestCaseReport.save_html"),
            ):
...
class TestEnsureT32Running(unittest.TestCase):
    ...
    def test_already_running_returns_immediately(self):
        ...
        import importlib
        main_mod = importlib.import_module("Automation_Framework.main")

        mock_cls, mock_conn = self._mock_conn_cls(try_connect_returns=True)
        with patch(
            "Automation_Framework.core.connection.T32Connection", mock_cls,
        ):
            # Should not raise.
    ...
    def test_not_running_auto_launch_launches_and_waits(self):
        ...
        with (
            patch("Automation_Framework.core.connection.T32Connection", mock_cls),
            patch("Automation_Framework.main._time.monotonic",
                  side_effect=[0.0, 0.5, settings.connect_max_wait_s + 1]),
            patch("Automation_Framework.main._time.sleep"),
        ):
...
class TestMainArgWiring(unittest.TestCase):
    ...
    def _import_main(self):
        import importlib
        return importlib.import_module("Automation_Framework.main")
