"""
Automation Framework – HTML Test Report Generator
=========================================================
...
report after each test run.  The report is saved to::
    ...
    <repo_root>/Automation_Framework/Test_Report/<YYYYMMDD_HHMM>/
...
The generated HTML is written to::
    ...
    <repo_root>/Automation_Framework/Test_Report/<YYYYMMDD_HHMM>/<suite_name>_report.html
...
def make_report_dir(base: Optional[Path] = None) -> Path:
    ...
    base:
        ...
        this module's location (the parent of ``Automation_Framework/``).
    ...
    Path
        The newly created (or already-existing) directory::
            ...
            <base>/Automation_Framework/Test_Report/<YYYYMMDD_HHMM>/
    """
    if base is None:
        # This file: <repo>/Automation_Framework/html_report.py
        # repo root: two levels up  → parent.parent
        base = Path(__file__).resolve().parent.parent

    stamp = datetime.datetime.now().strftime("%Y%m%d_%H%M")
    report_dir = base / "Automation_Framework" / "Test_Report" / stamp
...
# ---------------------------------------------------------------------------
# Report logo (inline SVG – no external dependency)
#
# Neutral framework mark:
#   • Red status dot on the left
#   • "T32 TEST" wordmark in grey (#9b9b9b)
# ---------------------------------------------------------------------------
...
_LOGO_SVG = (
    ...
    # ── Status dot ───────────────────────────────────────────────────────────
    '<circle cx="14" cy="33" r="10" fill="#e8230a"/>'
    # ── "T32 TEST" wordmark ──────────────────────────────────────────────────
    '<text x="34" y="47"'
    ' font-family="\'Arial Black\',\'Helvetica Neue\',Arial,sans-serif"'
    ' font-weight="900" font-size="38" letter-spacing="1" fill="#9b9b9b">'
    'T32 TEST</text>'
    '</svg>'
)
...
def render_html(
    ...
    _logo = _LOGO_SVG   # bring into local scope for f-string interpolation
...
<body>
  ...
  <div class="footer">
    Automation Framework &middot; {suite_name} &middot; {generated_at}
  </div>
</body>
</html>"""
...
class SanityHtmlRunner(unittest.TextTestRunner):
    ...
    report_dir:
        ...
        saved to ``<repo_root>/Automation_Framework/Test_Report/<stamp>/``.
    mode:
    ...
    **kwargs:
    ...
    def __init__(
        self,
        suite_name: str = "Framework_Sanity",
    ...
    def _save_report(self, result: _HtmlTestResult) -> None:
        ...
        print(f"\n[Framework] HTML report saved → {report_path}", file=sys.stderr)
