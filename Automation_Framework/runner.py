"""
Automation Framework – JSON-driven Test Runner
======================================================
...
def run_all_discovered(
    ...
    for json_path in files:
        ...
        print(f"\n[Framework] Running suite '{label}' from {json_path.name} …")
...
def run_from_json(
    ...
    if resilient_connect:
        ...
        if conn.try_connect():
            print(
                f"[Framework] Trace32 detected on port {settings.rcl_port} "
                "– connecting to existing instance."
            )
        elif auto_launch:
            print(
                f"[Framework] Trace32 not found on port {settings.rcl_port} "
        ...
        print(
            f"[Framework] Launching Trace32 on port {settings.rcl_port} …"
        )
        conn.launch()
    ...
    with conn:
        ...
        import Automation_Framework.core.debugger as _dbg
