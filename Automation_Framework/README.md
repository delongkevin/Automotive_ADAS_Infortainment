...
debugger in OEM automation test environments.
...
`OEM/OEM_A/Infotainment_Automation` directory.  It provides the same logical operations
...
- **Self-contained** HTML reports with an inline SVG header mark (no external
  URL required).
...
```python
import Automation_Framework as t32
from Automation_Framework import core
...
```python
from Automation_Framework import generate_from_live_session, T32Connection
...
```python
from Automation_Framework import discover_symbols, generate_from_inventory
...
```python
from Automation_Framework import discover_symbols, generate_from_live_session
from Automation_Framework.report import ModuleStatusReport
from Automation_Framework import runner
...
```python
from Automation_Framework import connect
...
```python
from Automation_Framework import auto_detect_t32, T32Connection
from Automation_Framework import core
...
```bash
# From the repository root
python -m pytest Automation_Framework/tests/ -v
# or
python -m unittest discover Automation_Framework/tests/
```
...
9. **Self-contained reports** – every HTML report carries an inline SVG header mark (no external URL required), with the suite name in the page title and footer.
