"""
simulate_tests.py  –  Virtual Bench Simulation for CVADAS SW Test
=======================================================================
Discovers all ADAS vTestStudio test sequences from the CANoe .run cache
...
1. Scans the .run directory under BVTRBS/CVADAS_RBS_TRSC for the
   latest .vtt.export.xml file for each known ADAS test unit.
...
Usage
-----
    python simulate_tests.py [--root <ADAS_SWTest folder>]
...
# ---------------------------------------------------------------------------
# Known ADAS test units (matches the CANoe test units configured in the
...
def _find_run_dir(adas_root: Path) -> Optional[Path]:
    """
...
def _write_html_report(
    ...
    """
    Write a single consolidated HTML simulation report for all ADAS suites.
    """
...
<head>
  <meta charset="UTF-8">
  <title>CVADAS – Simulation Report</title>
  <style>{_HTML_STYLE}</style>
</head>
<body>
  <h1>CVADAS ADAS SW Test – Simulation Report</h1>
  ...
  <p><strong>Generated :</strong> {generated}<br>
     <strong>Root      :</strong> {adas_root}<br>
  ...
  <p style="font-size:12px;color:#888;">
    Generated by OEM/OEM_B/ADAS_SWTest/simulate_tests.py &nbsp;|&nbsp;
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Virtual bench simulation for CVADAS ADAS SW Test. "
    ...
    parser.add_argument(
        ...
        help="Root directory of ADAS_SWTest (default: same folder as this script)",
    )
    parser.add_argument(
        '--out-dir', '-o',
        type=Path,
    ...
    # ------------------------------------------------------------------
    # Process each known ADAS test suite
    # ------------------------------------------------------------------
    print(f"CVADAS Virtual Bench Simulation")
    print(f"  Root    : {root}")
    ...
    # ------------------------------------------------------------------
    summary_lines = [
        "CVADAS ADAS SW Test – Virtual Bench Simulation Summary",
        "=" * 60,
