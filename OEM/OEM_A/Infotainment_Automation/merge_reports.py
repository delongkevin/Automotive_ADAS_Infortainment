"""
merge_reports.py  –  Consolidated HTML Report Generator for Infotainment Automation
==============================================================================
Merges all CANoe test-module XML reports and any Trace32 diagnostic XML
reports found under the Infotainment_Automation tree into a single, easy-to-read
HTML file.
...
3. Any additional  ``report.xml``  files found immediately inside the
   ``RBS/`` folder (the "all-tests" roll-up written by SWtest).

Usage
-----
    python merge_reports.py [--root <Infotainment_Automation folder>]
...
def discover_reports(root: Path, xml_dir: Optional[Path] = None) -> List[ReportModule]:
    ...
    Search order when *xml_dir* is None (determines display order in HTML):
      ...
      2. RBS/report.xml  (all-tests roll-up)
...
def generate_html(modules: List[ReportModule], generated_at: datetime.datetime,
    ...
    L.append("<title>Infotainment Automation \u2013 Test Report</title>")
    ...
    L.append('<h1>Infotainment Automation</h1>')
    ...
    L.append('<h1>Infotainment Automation \u2013 Test Report</h1>')
    ...
    L.append(
        ...
        f'Infotainment Automation &mdash; {html.escape(report_kind)} &mdash; '
        f'Generated {ts}'
        f'</div>'
    )
    L.append('</main>')
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="Root directory of Infotainment_Automation (default: same folder as this script)",
    )
    parser.add_argument(
        "--out", "-o",
        type=Path,
