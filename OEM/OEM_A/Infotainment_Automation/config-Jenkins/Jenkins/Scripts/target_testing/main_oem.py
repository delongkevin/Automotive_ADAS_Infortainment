"""
OEM Target Testing Script (APPL_GM)

This script handles on-target testing for the OEM library build mode (APPL_GM).
It performs:
1. UCB flash (ucbstart.ptp)
2. Application flash via start_headless.cmm (Boot.ptp, APPL_GM.hex, Calibration.ptp, EthSwitch)
...
# UART log keys for the APPL_GM target
UART_LOG_KEY_GM = 'uart_appl_gm'
UART_LOG_KEY_GM_EQ = 'uart_appl_gm_EQ'


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="OEM Target Testing (APPL_GM)")
    p.add_argument(
        "--report_folder",
        type=Path,
...
def _flash_gm_setup(dbg) -> None:
    """Flash UCB first, then the application via start_headless.cmm."""
    ...
    # Step 2: Flash the application (Boot, App, Cal, EthSwitch) via start_headless.cmm
    # This uses the GM_Scripts folder with SetGlobalVariables.cmm for proper memory map setup
    logger.info("Flashing application (Boot, App, Cal, EthSwitch)...")
...
def _copy_uart_logs_to(report_folder: Path) -> None:
    """Copy both UART logs to report folder."""
...
def main() -> Optional[int]:
    ...
    try:
        ...
        # Start UART logging (main UART)
        logger.info("Starting UART console...")
        ...
        logger.info("Target testing sequence completed.")
...
if __name__ == "__main__":
    ...
    start = time.time()
    code = main()
    logger.info(f"Target testing took {round(time.time() - start)} seconds")
