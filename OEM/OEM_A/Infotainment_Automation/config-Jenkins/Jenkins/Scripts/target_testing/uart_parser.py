...
TEST_CONFIG = {
    ...
    # EyeQ-specific tests (placeholders)
...
def test_eyeq_vision_mode(contents: str) -> Optional[int]:
    """
    Test: Check EyeQ vision mode status (APPL_GM EyeQ UART only).
    """
    modes = {
        'EyeQ': {'burn-code'},
        'APP': {'pending', 'running'}
...
def test_eyeq_warning_errors(contents: str) -> Optional[int]:
    """
    Test: Check for EyeQ-specific warning or error messages (APPL_GM EyeQ UART only).
    """
