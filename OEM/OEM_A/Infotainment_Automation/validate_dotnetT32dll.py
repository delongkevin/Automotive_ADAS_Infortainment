...
Usage
-----
    python validate_dotnetT32dll.py [--root <Infotainment_Automation folder>]

    The script deduces ``controlLib/T32/`` relative to ``--root``.  Run
    it from the repo root or pass ``--root`` explicitly:

        python OEM/OEM_A/Infotainment_Automation/validate_dotnetT32dll.py \\
            --root OEM/OEM_A/Infotainment_Automation
...
# Relative path inside Infotainment_Automation where CANoe loads the DLL from.
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help=(
            "Root directory of Infotainment_Automation "
